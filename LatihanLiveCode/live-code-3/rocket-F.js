...
function countLetterRecursive(sentence, letter) {
  if (sentence.charAt(sentence.length - 1) == "") {
    return 0;
  } else if (sentence.charAt(sentence.length - 1) == letter) {
    return (
      1 +
      countLetterRecursive(sentence.substring(0, sentence.length - 1), letter)
    );
  } else {
    return countLetterRecursive(sentence.substring(0, sentence.length - 1), letter);
  }
