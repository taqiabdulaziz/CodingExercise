...
function getReport(studentGrades) {
  ...
  for (let i = 0; i < kelas.length; i++) {
    JSON[i] = {
      classCode: kelas[i],
      passed: passFunc(kelas[i]),
      failed: failFunc(kelas[i])
    };
  }
  ...
  function passFunc(params) {
    ...
    for (let j = 0; j < studentGrades.length; j++) {
      if (
        studentGrades[j].classCode == params &&
        studentGrades[j].score >= 70
      ) {
        namePass.push(studentGrades[j].name);
      }
    }
  ...
  function failFunc(params) {
    ...
    for (let j = 0; j < studentGrades.length; j++) {
      if (
        ...
        nameFail.push(studentGrades[j].name);
      }
    }
