...
function getGuildMemberInfo(members) {
  ...
  median = Math.floor(totalLevel / members.length);

  for (let j = 0; j < members.length; j++) {
    if (kelas.length == 0 && members[j].level >= median) {
      kelas.push(members[j].class);
    }
    for (let k = 0; k < kelas.length; k++) {
      if (
        kelas.length != 0 &&
        members[j].level >= median &&
        members[j].class == kelas[k]
      ) {
        check = true;
      }
    }
    if (check == false && members[j].level >= median) {
      kelas.push(members[j].class);
    }
  ...
  for (let m = 0; m < kelas.length; m++) {
    for (let n = 0; n < members.length; n++) {
      if (members[n].level >= median && members[n].class == kelas[m]) {
        JSON[kelas[m]].push({
          name: members[n].name,
